...
mod tests {
    use super::*;
    use crate::game::{GameCell, CellState, GamePhase, GameSettings};
    use chrono::Utc;

    fn default_clock() -> crate::services::DefaultSystemClock {
        crate::services::DefaultSystemClock::default()
    }

    #[test]
    fn should_initialize_game_state() {
        let state = GameState::new(default_clock());
        let settings = GameSettings { rows: 5, columns: 5, mines_count: 3 };

        let new_state = state.play(settings.clone());

        match new_state.phase() {
            GamePhase::Initializing { cells, rows, columns, mines_count } => {
                assert_eq!(*rows, settings.rows);
                assert_eq!(*columns, settings.columns);
                assert_eq!(*mines_count, settings.mines_count);
                assert_eq!(cells.len(), settings.rows * settings.columns);
                assert!(cells.iter().all(|c| c.state == CellState::Hidden));
            }
            _ => panic!("Expected initializing phase"),
        }
    }

    #[test]
    fn should_toggle_flag() {
        let state = GameState::initializing(3, 3).reveal(0, 0);
        let new_state = state.toggle_flag(2, 2);

        if let GamePhase::Playing { cells, flags_count, .. } = new_state.phase() {
            let idx = 1 * 3 + 1;
            assert_eq!(cells[idx].state, CellState::Flagged);
