...
impl HttpResponse {
    pub async fn into_blob(self) -> Result<Blob, AppError> {
        let js_value = JsFuture::from(self.0.blob()?)
            .await
...
impl HttpClient {
    ...
    pub async fn get_as_blob(&self, url: &str) -> Result<Blob, AppError> {
        self.get_with_cors(url).await?.into_blob().await
    }
